       ...
       // private readonly IMapper Mapper;
        ...
        public ManualInViewModel()
        {
          ...
          //  Mapper = IocResolver.Resolve<IMapper>();
            ...
            StartAutoRandomCommand = new RelayCommand(StartAutoRandom);
            StartAutoOrderCommand = new RelayCommand(StartAutoOrder);
            CloseAutoRandomCommand = new RelayCommand(CloseAutoRandom);
        ...
        public RelayCommand StartAutoRandomCommand { get; private set; }

        public RelayCommand StartAutoOrderCommand { get; private set; }

        public RelayCommand CloseAutoRandomCommand { get; private set; }
        ...
        private static bool isAutoRandom = true;

        private static bool isAutoOrder = true;
        ...
        public async Task StartAuto()
        {
            ...
            while (!IsAutoStart)
            {
                ...
                else
                {
                    ...
                    if (alarm != null)
                    {
                        var AlarmStatusCap = alarm.AlarmStatusDescription;
                        // 获取当前时间
                        DateTime nowes = DateTime.Now;
                        // 创建一个字符串，用于记录文本内容
                        string textes = $"设备报警时间：{nowes:yyyy-MM-dd HH:mm:ss}   设备报警内容：{AlarmStatusCap}\r\n";
                        // 设置文本文件的路径和文件名
                        string filePathes = "C:\\autoRandom.txt";
                        // 将text字符串写入到文本文件中
                        File.AppendAllText(filePathes, textes);
                    }
                }

                if (query.AlarmStatus != (int)DeviceAlarmStateEnum.Urgencye)
                {
                    StartAutoInRandomTraycode();
                    await Task.Delay(60000);
                }
                else
                {
        ...
        public async void StartAutoRandomTraycode()
        {
            if(isAutoRandom)
            {
                isAutoOrder = false;
                ...
                await AutoChangeValue();

                try
                {
                    string CurrentRunningTray = randomNumber.ToString();
                    ...
                    {
                        var TakeInTrayNumber = runingEntity.TrayCode.ToString();
                        // 获取当前时间
                        DateTime now = DateTime.Now;
                        // 创建一个字符串，用于记录文本内容
                        string text = $"取出托盘时间：{now:yyyy-MM-dd HH:mm:ss}   取出托盘号：{TakeInTrayNumber}\r\n";
                        // 设置文本文件的路径和文件名
                        string filePath = "C:\\autoRandom.txt";
                        // 将text字符串写入到文本文件中
                        File.AppendAllText(filePath, text);

                        string cfgINI = AppDomain.CurrentDomain.BaseDirectory + SerivceFiguration.INI_CFG;
                        IniFile ini = new IniFile(cfgINI);
                        ini.IniWriteValue("ClientInfo", "CurrentRunningTray", TakeInTrayNumber);
            ...
            {
                Msg.Warning("请勿连续开启服务，等待托盘回位后再开启！");
            }
        ...
        /// 自动存入托盘
        /// </summary>
        public async void StartAutoInRandomTraycode()
        {
            try
            {
                isAutoOrder = true;
                ...
                if (!string.IsNullOrEmpty(CurrentRunningTray))
                {
                    ...
                    if (runningContainer.Success)
                    {
                        ...
                        DateTime now = DateTime.Now;
                        // 创建一个字符串，用于记录文本内容
                        string text = $"存入托盘时间：{now:yyyy-MM-dd HH:mm:ss}   存入托盘号：{TakeInTrayNumber}\r\n";
                        // 设置文本文件的路径和文件名
                        string filePath = "C:\\autoRandom.txt";
                        // 将text字符串写入到文本文件中
                        File.AppendAllText(filePath, text);
        ...
        /// 随机存取
        /// </summary>
        private string _isAutoStartString = "开启随机存取";
        public string IsAutoStartString
        {
            get { return _isAutoStartString; }
            set { _isAutoStartString = value; RaisePropertyChanged();}
        }
        
        public async Task AutoChangeValue()
        {

            IsAutoStartString = "已开启";
            
            TakeInTrayNumber = randomNumber.ToString();
        }

        public async Task AutoChangeValuees()
        {

            IsOrderStartString = "已开启";
        ...
        /// 开启自动随机存取托盘
        /// </summary>
        public async void StartAutoRandom()
        {
            if (IsAutoStart && !lockDown)
            {
                IsAutoStart = false;

                string cfgINI = AppDomain.CurrentDomain.BaseDirectory + SerivceFiguration.INI_CFG;
                IniFile ini = new IniFile(cfgINI);
                ini.IniWriteValue("ClientInfo", "IsAutoStart", IsAutoStart.ToString());
        ...
        /// 关闭自动存取托盘
        /// </summary>
        public async void CloseAutoRandom()
        {
            if (!await Msg.Question("是否关闭自动存取托盘？"))
            {
                return;
            }
            ...
            lockDown = false;
            if(IsAutoStartString == "已开启")
            {
                IsAutoStartString = "开启随机存取";
            }
            if (IsOrderStartString == "已开启")
            {
                IsOrderStartString = "开启顺序存取";
            }
        }
        ...
        /// 顺序存取
        /// </summary>
        private string _isOrderStartString = "开启顺序存取";
        public string IsOrderStartString
        {
            get { return _isOrderStartString; }
            set { _isOrderStartString = value; RaisePropertyChanged(); }
        }

        /// <summary>
        /// 开启自动顺序存取托盘
        /// </summary>
        public async void StartAutoOrder()
        {
            if (IsAutoStart && !lockDown)
            {
                IsAutoStart = false;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task StartAutoOrderes()
        {
            var query = WareHouseContract.Containers.FirstOrDefault(a => a.Code == ContainerCode);
            ...
            while (!IsAutoStart)
            {
                if (query.AlarmStatus != (int)DeviceAlarmStateEnum.Urgencye)
                {
                    foreach (int num in Enumerable.Range(MinValue, MaxValue))
                    {
                        if(!IsAutoStart)
                        {
                            randomNumber = num;
                            StartAutoRandomTraycodees();
                            await Task.Delay(60000);
                            StartAutoInRandomTraycodees();
                            await Task.Delay(60000);
                        }
                        else
                        {
        ...
        public async void StartAutoRandomTraycodees()
        {
            if(isAutoOrder)
            {
                isAutoRandom = false;
        ...
        public async void StartAutoInRandomTraycodees()
        {
            try
            {
                isAutoRandom = true;
